...
import type { CategoryForExtraction, ExtractResponse } from "../types";
...
// module), per explicit instruction.
...
// (one page, or several merged pages), sent as a native document/image
// block — Claude's vision API accepts PDF documents directly, so the
// backend never needs to rasterize a page to an image itself.
...
Call the ${EXTRACTION_TOOL_NAME} tool with your result. If nothing matches, use an empty suggestedCategoryName and leave extractedFields/redFlags empty.`;
  ...
  try {
    ...
    const redFlags = (input.redFlags ?? [])
      ...
      .filter((flag): flag is { categoryFieldId: number; triggered: boolean; reason: string } => flag !== null);
