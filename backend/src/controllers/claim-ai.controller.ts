...
import { AiExtractionLog, Category, CategoryField, Claim, ClaimInvoiceFile, Expense } from "../models";
import { extractInvoiceData, type CategoryForExtraction } from "../services/ai-extraction.service";
import {
...
// run through the AI/ML service end to end: log the attempt, extract, then
// create the resulting Expense — blank if extraction failed or returned
// nothing usable, per 023's own "source still reaches Review, blank" rule.
async function runSourceExtraction(params: {
  ...
  let rawRequestSummary: Record<string, unknown> | null = null;
  let rawModelResponse: Record<string, unknown> | null = null;
  ...
  try {
    ...
    if ("error" in result) {
      ...
      rawRequestSummary = result.rawRequestSummary;
    ...
    errorMessage = err instanceof Error ? err.message : "AI/ML extraction failed.";
  }
  ...
  log.expenseId = expense.id;
  log.respondedAt = new Date();
  log.rawRequestSummary = rawRequestSummary;
  log.rawModelResponse = rawModelResponse;
  log.suggestedCategoryId = suggestedCategoryId;
  log.confidence = confidence !== null ? confidence.toFixed(4) : null;
  log.redFlagEvaluations = null;
  log.status = errorMessage ? "failed" : "completed";
...
async function runExtractionForClaim(claimId: number): Promise<void> {
  ...
  const alreadyLogged = await AiExtractionLog.findAll({ where: { claimInvoiceFileId: invoiceFiles.map((file) => file.id) } });
...
export async function getProcessingStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  const logs = await AiExtractionLog.findAll({ where: { claimInvoiceFileId: invoiceFiles.map((file) => file.id) } });
