...
Next.js (App Router, TypeScript, `src/` directory, import alias `@/*`), styled with Tailwind CSS 4 + shadcn/ui. Implements login/forgot-password (`user-stories/001-authentication.md`), the 5-step company registration wizard (`user-stories/002-organization-signup.md`), the post-login header navigation + switch-organization screen (`user-stories/003-header-navigation.md`), and Grade Management (`user-stories/004-grade-management.md`). Structure is modeled on `zp-frontend`'s conventions (see `../../nextjs/zp-frontend/CLAUDE.md`), scaled down to what this app actually has — folders aren't pre-created until there's real content for them.
...
One subfolder per domain, one file per endpoint: `src/apis/auth/` (login/logout/me/forgot-password/registration endpoints — registration's endpoints are still `auth/*` since they're all under the backend's `/api/auth/registrations/*` namespace), `src/apis/organization/` (GST-availability check + `listMine.api.ts` for `GET /api/organizations/mine`), `src/apis/user/` (`switchActiveOrganization.api.ts` for `PATCH /api/users/me/active-organization` — a separate domain from `organization/` since the endpoint itself is under the backend's `/api/users/*` namespace, not `/api/organizations/*`), and `src/apis/grade/` (list/create/update/updateStatus/delete/listMembers, one file each, matching the backend's `/api/grades*` routes one-to-one), each just calling `apiCall`/`postJson` from `src/utils/apiManager/apiManager.ts` and returning its typed response. Every domain folder has an `index.ts` barrel; import from the domain barrel (`@/apis/auth`, `@/apis/organization`, `@/apis/user`, `@/apis/grade`), not individual `.api.ts` files, outside the domain itself. No query-key layer yet — this project doesn't use a data-fetching/cache library (no React Query), so there's nothing to key. If one gets added later, add `src/apis/queryKeys/<domain>.queryKeys.ts` to match `zp-frontend`'s convention at that point.

**Server-driven search/sort/pagination screens (Grade Management, and its siblings to come) reset loading/error state synchronously at the top of an effect before refetching** — the standard shape for every future org-scoped CRUD screen (`src/app/(private)/company-settings/grades/page.tsx` is the reference implementation). `eslint.config.mjs` turns off `react-hooks/set-state-in-effect` project-wide for exactly this reason: without a data-fetching library, "fetch on a dependency change, resetting loading/error first" is the correct pattern (React's own docs list data-fetching as a valid effect use), and the rule's concern is an extra render pass, not a bug. Don't silence it per-line instead of relying on this — the whole point of turning it off centrally is that every future CRUD screen hits the exact same shape.
