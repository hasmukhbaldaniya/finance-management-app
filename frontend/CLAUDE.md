...
Next.js (App Router, TypeScript, `src/` directory, import alias `@/*`), styled with Tailwind CSS 4 + shadcn/ui. Implements login/forgot-password (`user-stories/001-authentication.md`), the 5-step company registration wizard (`user-stories/002-organization-signup.md`), the post-login header navigation + switch-organization screen (`user-stories/003-header-navigation.md`), Grade Management (`user-stories/004-grade-management.md`), and Department Management (`user-stories/005-department-management.md`). Structure is modeled on `zp-frontend`'s conventions (see `../../nextjs/zp-frontend/CLAUDE.md`), scaled down to what this app actually has — folders aren't pre-created until there's real content for them.
...
```
`src/hooks/` holds one hook so far: `useInfiniteScroll.ts` — created once Grade and Department Management both needed the identical "load the next page when the user scrolls to the bottom" behavior (an `IntersectionObserver` watching a sentinel `<div>`, no "Load more" button). It reads `onLoadMore`/`hasMore`/`isLoading` via a ref updated in an effect (not during render — `react-hooks/refs` flags writing a ref synchronously in the render body) so the observer itself is created once per mount instead of being torn down and recreated on every fetch. Reuse this for every future paginated list (Roles next), don't re-implement it inline. No `src/icons/` yet — not created until there's a real shared icon component to put in it (see `zp-frontend/CLAUDE.md`'s own layer description for what it's for when that day comes).

### `src/apis/` — API layer

One subfolder per domain, one file per endpoint: `src/apis/auth/` (login/logout/me/forgot-password/registration endpoints — registration's endpoints are still `auth/*` since they're all under the backend's `/api/auth/registrations/*` namespace), `src/apis/organization/` (GST-availability check + `listMine.api.ts` for `GET /api/organizations/mine`), `src/apis/user/` (`switchActiveOrganization.api.ts` for `PATCH /api/users/me/active-organization` — a separate domain from `organization/` since the endpoint itself is under the backend's `/api/users/*` namespace, not `/api/organizations/*`), `src/apis/grade/`, and `src/apis/department/` (each: list/create/update/updateStatus/delete/listMembers, one file each, matching the backend's `/api/grades*`/`/api/departments*` routes one-to-one — `department/` is a straight copy of `grade/` with the noun swapped, nothing else differs), each just calling `apiCall`/`postJson` from `src/utils/apiManager/apiManager.ts` and returning its typed response. Every domain folder has an `index.ts` barrel; import from the domain barrel (`@/apis/auth`, `@/apis/organization`, `@/apis/user`, `@/apis/grade`, `@/apis/department`), not individual `.api.ts` files, outside the domain itself. No query-key layer yet — this project doesn't use a data-fetching/cache library (no React Query), so there's nothing to key. If one gets added later, add `src/apis/queryKeys/<domain>.queryKeys.ts` to match `zp-frontend`'s convention at that point.
...
- `src/app/(private)/` — authenticated screens: `dashboard/`, `trips/`, `claims/`, `approvals/`, `finance/`, `reports/`, `help/`, `profile/`, and `company-settings/{employees,categories,roles-privileges,grades,departments,organizations}/`. All but `dashboard/`, `company-settings/organizations/`, `company-settings/grades/`, and `company-settings/departments/` are still placeholder pages (render `<ComingSoon title="..." />` from `src/components/coming-soon.tsx`) — see `user-stories/003-header-navigation.md`'s Out of Scope for which destinations are real. `company-settings/grades/page.tsx` and `.../departments/page.tsx` are the reference implementations for every future org-scoped CRUD screen (Roles next) — see `src/components/grade/` and `src/components/department/` (`form-dialog.tsx`/`members-dialog.tsx`/`status-dialog.tsx` each) for the Add/Edit-with-Delete-inside, Members-list, and toggle-confirmation dialog patterns to reuse. **The status toggle always opens a confirmation dialog before changing anything** — `grade/status-dialog.tsx`/`department/status-dialog.tsx` — there is no optimistic/instant toggle anywhere in this codebase, even though [004](../user-stories/004-grade-management.md)'s story doc originally specced one; that decision was superseded after implementation (see that doc's Open Questions) and Department was built with the confirmation dialog from the start. Gated by `src/proxy.ts`'s `matcher`; any new authenticated route must be added under `(private)/` **and** to that matcher array. `(private)/layout.tsx` wraps every page in `SessionProvider` (see `src/contexts/`), which does the one `getMe()` fetch for the whole private area and renders the shared `Header` — individual pages must not re-fetch `getMe()` or render their own header/logout, they consume `useSession()` instead (see `dashboard/page.tsx` for the pattern).
...
- **Components unique to one feature get their own `<feature>/` folder** — `src/components/grade/{form-dialog,members-dialog,status-dialog}.tsx` and `src/components/department/{form-dialog,members-dialog,status-dialog}.tsx` (imported as `GradeFormDialog` from `@/components/grade/form-dialog`, etc. — the feature name lives in the folder, not repeated in the filename). This revisits the flat-components decision above: it made sense with one feature area (auth) and a handful of files, but stopped making sense once Grade and Department each grew 3 tightly-coupled dialog components sitting flat alongside everything else. Follow this shape for Roles next, not the old flat one — a `<feature>/` folder is the default the moment a feature needs more than one component of its own, not something to defer indefinitely.
