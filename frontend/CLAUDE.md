...
Next.js (App Router, TypeScript, `src/` directory, import alias `@/*`), styled with Tailwind CSS 4 + shadcn/ui. Implements login/forgot-password (`user-stories/001-authentication.md`), the 5-step company registration wizard (`user-stories/002-organization-signup.md`), the post-login header navigation + switch-organization screen (`user-stories/003-header-navigation.md`), Grade Management (`user-stories/004-grade-management.md`), Department Management (`user-stories/005-department-management.md`), and Roles & Privileges Management (`user-stories/006-roles-and-privileges-management.md`). Structure is modeled on `zp-frontend`'s conventions (see `../../nextjs/zp-frontend/CLAUDE.md`), scaled down to what this app actually has — folders aren't pre-created until there's real content for them.
...
- **Components unique to one feature get their own `<feature>/` folder** — `src/components/grade/`, `src/components/department/` (each: `form-dialog,members-dialog,status-dialog`), and `src/components/role/` (those three plus `view-dialog.tsx` and `privilege-checkbox-list.tsx`) — imported as `GradeFormDialog` from `@/components/grade/form-dialog`, etc. — the feature name lives in the folder, not repeated in the filename. This revisits the flat-components decision above: it made sense with one feature area (auth) and a handful of files, but stopped making sense once Grade/Department/Roles each grew several tightly-coupled dialog components. Follow this shape for Employees next, not the old flat one — a `<feature>/` folder is the default the moment a feature needs more than one component of its own, not something to defer indefinitely.
