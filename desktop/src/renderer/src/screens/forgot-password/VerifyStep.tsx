// Ported from frontend/src/app/(public)/forgot-password/verify/page.tsx.
...
export function ForgotPasswordVerifyStep() {
  ...
  useEffect(() => {
    if (!email && !isLeavingRef.current) {
      navigate(ROUTES.FORGOT_PASSWORD.REQUEST, { replace: true });
    }
  }, [email, navigate]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      navigate(ROUTES.FORGOT_PASSWORD.RESET);
    } catch (err) {
    ...
    <AuthCard title="Verify OTP" description={`Enter the 6-digit code sent to ${email}.`}>
      <Stack component="form" onSubmit={handleSubmit} noValidate spacing={2}>
        ...
        <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}>
          <MuiLink component={Link} to={ROUTES.FORGOT_PASSWORD.REQUEST} underline="hover" variant="body2">
            ← Change email
          </MuiLink>
          <MuiLink component={Link} to={ROUTES.LOGIN} onClick={leaveAndReset} underline="hover" variant="body2">
            Back to Login
          </MuiLink>
        </Stack>
      </Stack>
