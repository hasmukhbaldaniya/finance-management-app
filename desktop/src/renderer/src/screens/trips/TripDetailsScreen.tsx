// Ported from frontend/src/app/(private)/trips/[id]/page.tsx.
// next/link -> react-router's Link (href= -> to=); next/navigation's
// useParams -> react-router's useParams (identical shape).
  ...
  return (
    <Stack spacing={3} sx={{ mx: "auto", maxWidth: 1152, px: 3, py: 3 }}>
      <Stack component="nav" direction="row" spacing={1} aria-label="Breadcrumb" sx={{ alignItems: "center", borderBottom: 1, borderColor: "divider", pb: 2, fontSize: "0.875rem" }}>
        <MuiLink component={Link} to={ROUTES.TRIPS} color="text.secondary" sx={{ display: "flex", alignItems: "center", gap: 0.75, "&:hover": { color: "text.primary" } }}>
