...
export function RegisterDetailsScreen() {
  ...
  useEffect(() => {
    if (!organizationName || !gstNumber) {
      navigate(ROUTES.REGISTER.ORGANIZATION, { replace: true });
    }
  }, [organizationName, gstNumber, navigate]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      navigate(ROUTES.REGISTER.VERIFY_EMAIL);
    } catch (err) {
      ...
      if (err instanceof ApiError && err.status === 409 && message.includes("GST")) {
        navigate(ROUTES.REGISTER.ORGANIZATION);
