// Ported from frontend/src/app/(private)/company-settings/employees/page.tsx.
// next/link -> react-router's Link (href= -> to=), the `component={Link}`
// MUI composition pattern unchanged. Everything else is verbatim, including
// the asymmetric Suspend-confirms/Activate-doesn't toggle and the listing
// excluding the caller's own row (enforced server-side, nothing to port here).
    ...
    <Box sx={{ mx: "auto", display: "flex", minHeight: 0, width: "100%", maxWidth: 1152, flex: 1, flexDirection: "column", overflow: "hidden", px: 2, py: 5 }}>
      <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", gap: 2 }}>
        ...
        <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
          ...
          <Button component={Link} to={ROUTES.EMPLOYEE_BULK_INVITE} variant="outline">
            Bulk Invite
          </Button>
          <Button component={Link} to={ROUTES.EMPLOYEE_INVITE}>Invite</Button>
        </Stack>
      </Stack>
                    ...
                    <TableRow key={employee.id}>
                      ...
                      <TableCell>
                        <Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}>
                          ...
                          <MuiLink
                            component={Link}
                            to={ROUTES.employeeEdit(employee.id)}
