// Ported from frontend/src/app/(private)/company-settings/employees/bulk-invite/page.tsx.
// next/link -> react-router's Link; next/navigation's useRouter -> useNavigate.
// This is the first real exercise of the upload/download IPC path from
// apiManager.ts (uploadBulkImport/downloadBulkTemplate/downloadBulkImportErrors)
// — nothing else in this file changes, since apiManager.ts's rewritten
// primitives already hide the IPC boundary from every caller.
...
export function EmployeeBulkInviteScreen() {
      ...
      <Paper variant="outlined" sx={{ mt: 3, borderRadius: 2, p: 3 }}>
        <Stack spacing={3}>
          ...
          <Stack direction="row" spacing={1} sx={{ justifyContent: "flex-end", pt: 1 }}>
            <Button component={Link} to={ROUTES.COMPANY_SETTINGS.EMPLOYEES} variant="outline">
              Cancel
            </Button>
