// Ported from frontend/src/app/(private)/company-settings/employees/[id]/edit/page.tsx.
// next/link -> react-router's Link; next/navigation's useParams/useRouter ->
// react-router's useParams/useNavigate (useParams' shape is identical —
// Record<string, string | undefined> — so only the import site changes).
// A near-duplicate of EmployeeInviteScreen per 009's own Flow note that Edit
// "reuses [008]'s field-level rules verbatim": pre-filled from
// GET /employees/:id, every write updates instead of creates, no terminal
// "Send Invite" step.
...
export function EmployeeEditScreen() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      navigate(ROUTES.COMPANY_SETTINGS.EMPLOYEES);
    } catch (error) {
      toast.error(error instanceof ApiError ? error.message : GENERIC_ERROR_MESSAGE);
    } finally {
        ...
        <Stack direction="row" spacing={4} sx={{ mt: 3 }}>
          ...
          <Stack component="form" onSubmit={handleSubmit} noValidate spacing={3} sx={{ minWidth: 0, flex: 1 }}>
            ...
            <Stack direction="row" spacing={1} sx={{ justifyContent: "flex-end" }}>
              <Button component={Link} to={ROUTES.COMPANY_SETTINGS.EMPLOYEES} variant="outline" size="sm">
                Cancel
              </Button>
