// Ported from frontend/src/app/(private)/company-settings/associated-organizations/page.tsx
// verbatim — zero Next imports. Owner-only, read-mostly network listing with
// its own one-off filter-row UI pattern (not the Grade/Department/Role
// dialog shape) and an instant/optimistic status pill toggle — deliberately
// *not* confirm-gated, unlike every other status toggle in this cluster.
// Preserve that asymmetry; it's intentional upstream, not a bug to "fix".
...
export function AssociatedOrganizationsScreen() {
