...
type ButtonProps = Omit<MuiButtonProps, "variant" | "color" | "size"> & {
  ...
  size?: ButtonSize;
  // Ported call sites compose this with react-router's <Link> via
  // `component={Link} to={...}` (frontend/'s own equivalent used next/link's
  // `href`, which MuiButtonProps already types natively since Button can
  // render as an anchor — `to` is react-router's own prop name, which MUI
  // has no way to know about). Narrow, deliberate addition — not a general
  // escape hatch.
  to?: string;
};
