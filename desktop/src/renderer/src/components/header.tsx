// Ported from frontend/src/components/header.tsx. next/link's Link (href=)
// -> react-router's Link (to=); next/navigation's usePathname/useRouter ->
// react-router's useLocation().pathname / useNavigate(). Everything else —
// including the two distinct nav patterns (persistent sub-header links vs.
// the Profile dropdown) and the owner-only Associated Organizations link —
// is unchanged.
        ...
        <Stack direction="row" spacing={3} sx={{ alignItems: "center" }}>
          <Stack component="nav" direction="row" spacing={0.5} sx={{ alignItems: "center" }}>
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Box key={link.href} component={Link} to={link.href} aria-current={isActive ? "page" : undefined} sx={navLinkSx}>
                  {link.label}
                </Box>
              );
            })}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem render={<Link to={ROUTES.PROFILE}>View Profile</Link>} />
      ...
      {isOnCompanySettings ? (
        <Stack component="nav" direction="row" spacing={0.5} aria-label="Company Settings" sx={{ alignItems: "center", borderBottom: 1, borderColor: "divider", bgcolor: "action.hover", px: 2, py: 1 }}>
          {companySettingsLinks.map((link) => (
            <Box key={link.href} component={Link} to={link.href} aria-current={pathname === link.href ? "page" : undefined} sx={subNavLinkSx}>
              {link.label}
            </Box>
          ))}
        </Stack>
