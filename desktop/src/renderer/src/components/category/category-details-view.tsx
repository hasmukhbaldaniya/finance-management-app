// Ported from frontend/src/components/category/category-details-view.tsx.
// next/link -> react-router's Link; next/navigation's useRouter -> useNavigate;
// useSearchParams -> react-router's useSearchParams, whose first tuple element
// is a real URLSearchParams with the identical .get() method, so only the
// destructuring changes, not the usage.
...
export function CategoryDetailsView({ categoryId }: CategoryDetailsViewProps) {
  ...
  function handleVersionChange(version: string): void {
    navigate(`${ROUTES.categoryDetails(categoryId)}?version=${version}`);
  }
  ...
  return (
    <Box sx={{ mx: "auto", maxWidth: 1152, px: 3, py: 4 }}>
      <Stack direction="row" spacing={1.5} sx={{ mb: 3, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
        <Typography variant="body2" color="text.secondary">
          <MuiLink component={Link} to={ROUTES.COMPANY_SETTINGS.CATEGORIES} sx={{ "&:hover": { textDecoration: "underline" } }}>
