// Ported from frontend/src/components/category/version-history-drawer.tsx.
// next/link -> react-router's Link (href= -> to=).
      ...
      <DialogContent sx={{ width: "100%", maxWidth: { sm: 448 } }}>
        ...
        {isLoading ? (
          ...
          <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", borderRadius: 2, border: 1, borderColor: "divider", p: 1.5 }}>
            ...
            {category ? (
              <MuiLink component={Link} to={ROUTES.categoryDetails(category.id)} variant="body2" sx={{ fontWeight: 500, "&:hover": { textDecoration: "underline" } }}>
                View Details
              </MuiLink>
            ) : null}
          </Stack>
            ...
            {versions.map((version) => (
              ...
              >
                ...
                {category ? (
                  <MuiLink
                    component={Link}
                    to={`${ROUTES.categoryDetails(category.id)}?version=${version.version}`}
                    variant="body2"
