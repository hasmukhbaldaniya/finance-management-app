// Ported from frontend/src/components/category/category-card.tsx.
// next/link -> react-router's Link (href= -> to=, query string kept as a
// plain suffix on the string, which react-router parses the same way);
// next/navigation's useRouter -> useNavigate.
...
export function CategoryCard({ category, onOpenVersionHistory, onDelete, onToggleEnabled }: CategoryCardProps) {
  ...
  function handleCardClick(event: React.MouseEvent): void {
    ...
    navigate(ROUTES.categoryDetails(category.id));
  }
    ...
    >
      ...
      <Stack direction="row" data-stop-card-navigation sx={{ mt: "auto", alignItems: "center", justifyContent: "space-between", borderTop: 1, borderColor: "divider", pt: 1.5 }}>
        <Stack direction="row" spacing={0.5}>
          <Box component={Link} to={`${ROUTES.CATEGORY_NEW}?duplicateFrom=${category.id}`} aria-label={`Duplicate ${category.name}`} title="Duplicate" sx={iconButtonSx}>
            <CopyIcon size={16} />
          </Box>
          <Box component={Link} to={ROUTES.categoryStep(category.id, "basic-details")} aria-label={`Edit ${category.name}`} title="Edit" sx={iconButtonSx}>
