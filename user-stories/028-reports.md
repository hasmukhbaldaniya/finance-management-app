...
**By Employee view added** (later, following a broader architecture-review pass — not part of the original scope above, added on explicit request after a non-owner employee asked why their own claims didn't show up on a report they didn't have permission to view at all): every report's filter row now has a two-way toggle (MUI `ToggleButtonGroup`) switching between its original row-level view and a new "By Employee" view — one row per employee, with a count column (Expense/Claim/Trip Count) and Total Amount, sorted by Total Amount descending, same convention as every other table in this doc.

- **Claim Cost / Trip Cost / Red-Flagged Expenses**: the toggle relabels as `Detail | By Employee`. The By Employee rows are computed **client-side**, via a new shared `groupByEmployee()` helper (`frontend/src/utils/helpers/format.helper.ts`) that reduces the rows already fetched for the Detail view — no extra API call, the toggle is instant. Each report component holds its own `view` state; switching it re-renders the same fetched `rows` through `groupByEmployee`, it doesn't refetch.
- **Expense Summary**: relabels as `By Category | By Employee` (this report never had a per-expense detail view — it only ever returned category-level aggregates). Since the frontend never receives individual expense rows for this report, the By Employee breakdown had to be computed **server-side** instead: `reports-service`'s `getExpenseSummary` (`reports.controller.ts`) now always fetches `fetchAllEmployees` (previously only fetched when a `department` filter was set) and returns an additional `byEmployeeRows` array in the response, grouped by `employeeName` the same way `groupByEmployee` does client-side for the other three reports. `expenseSummary.api.ts`'s return type gained `byEmployeeRows: EmployeeSummaryRow[]` to match.
- To add this same toggle to a future report: reuse `groupByEmployee(rows, getEmployeeName, getAmount)` if the report already returns row-level data with an employee name attached (the common case); only reach for a server-side `byEmployeeRows` field like Expense Summary's if the report's `rows` are pre-aggregated and don't carry a per-employee dimension at all.
- Verified live in browser across all four reports, including that Claim Cost's By Employee total for a given employee matches the sum of that employee's own claims one-for-one. `tsc`/`eslint` clean on every changed file.
