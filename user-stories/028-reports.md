...
Covers the "Reports" nav destination (currently a bare `<ComingSoon title="Reports" />` at `frontend/src/app/(private)/reports/page.tsx`, per [003-header-navigation.md](./003-header-navigation.md)) — three read-only, organization-wide reports a Company Administrator uses to see spend and trip activity across every employee, not just their own. This is Phase 5 of `docs/PLANS/microservices-plan.md`, built as a new `reports-service` with no database of its own, aggregating over `auth-service`'s and `claim-service`'s own APIs (`docs/PLANS/microservices-frontend-integration-plan.md` section 1.1/1.4e).
