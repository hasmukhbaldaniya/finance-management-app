...
1. **This doc is the deliverable for now** — a full spec, matching how prior epics in this codebase were handled (write the story, implement in a later, separate pass).
2. **Phased-by-layer rollout**: theme/colors foundation → core primitives → composed components → feature-by-feature page re-styling (the largest, riskiest phase, done last). Tailwind and MUI coexist during the middle phases; Tailwind is only removed once the last phase confirms zero remaining references.
3. **Toast notifications migrate from `sonner` to MUI's `Snackbar`+`Alert`**, for full visual consistency with the new design system, rather than keeping the standalone `sonner` package restyled to the new palette.
...
| `dropdown-menu.tsx` (`DropdownMenu` family) | MUI `Menu`, `MenuItem` | The existing **checkbox multi-select pattern** (`DropdownMenuCheckboxItem` with `closeOnClick={false}`, used in `policy-eligibility-section.tsx`/`ziptrrip-category-picker.tsx`) maps cleanly onto MUI's own documented "`Autocomplete` with checkboxes, `disableCloseOnSelect`" recipe — a clean upgrade, not just a swap, since `Autocomplete` also gives free type-ahead filtering the current `DropdownMenu` version doesn't have. |
...
| `Calendar`→(removed, folded into `DatePicker`), `Popover`, `Select`, `Textarea`→`TextField` (`multiline`) | **No story today** (pre-existing gap) | New story added as part of this migration, not deferred further. |
