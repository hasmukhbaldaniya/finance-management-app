...
- **"Company Administrator" enforcement — now resolved and actually implemented**: at the time this doc was originally written, no such check existed anywhere beyond [007](./007-associated-organizations-network.md)'s owner gate. Since then, the User → Employee merge (see `backend/CLAUDE.md`) dropped `OrganizationMember` entirely and replaced `role === "owner"` with `Employee.isOwner`, and [007](./007-associated-organizations-network.md)'s `requireOwner` middleware already enforces exactly that. This story's 4 endpoints are gated by that same `requireOwner` middleware — "Company Administrator" means `Employee.isOwner`, consistently with [007](./007-associated-organizations-network.md), not [006](./006-roles-and-privileges-management.md)'s still-unenforced "Company Admin" `Role`.
- **`BulkUpload`/`BulkUploadError` use INTEGER autoincrement ids, not the UUIDs originally specified above** — every other table in this codebase uses INTEGER autoincrement; adding the one UUID primary key in the schema would be inconsistent for no functional benefit at this app's scale.
- **The parsed-but-unconfirmed rows between `/upload` and `/import` are held in an in-process `Map`, not a database table or cache** — this codebase has no Redis/queue anywhere to reach for instead. It doesn't survive a server restart and wouldn't work behind more than one backend instance; a real production version would need a proper store. Sessions expire after 30 minutes.
- **`Company` (a required per-row column) resolves against the caller's own organization's name only** — every other endpoint in this codebase scopes all writes to the caller's single active organization with no way to target a different one via the API; a bulk upload naming a *different* company than the one the admin is logged into is treated as a validation failure ("Company not found."), not a cross-organization create. This is a documented interpretation of an otherwise ambiguous column, not a partial implementation.
- **Matching priority order (Email → Contact Number → Employee ID) is scoped to the caller's own organization** — a match against a *different* organization's employee is not "the same person moving companies," it's a hard conflict, since `Employee.email`/`(countryCode, contactNumber)` are globally unique (see the User → Employee merge). A create-candidate row colliding with another organization's employee fails validation instead of silently creating a duplicate-email row that the DB would reject anyway. If two of the three fields disagree about which *in-organization* employee a row matches, the first match in priority order (Email) wins — this specific tie-break wasn't explicit in the source spec and is filled in here for determinism.
- **5,000-row / 10 MB limits and "acceptable performance"** are carried over as-specified from the source request; no concrete latency/timeout target is defined here (e.g. "must complete within N seconds") — flagged as something to pin down with a real number before load-testing against it. Row validation does batch its DB lookups (one query per lookup table, not per row) specifically so this stays reasonable at 5,000 rows.
...
- **The npm `xlsx` (SheetJS) package was deliberately avoided** in favor of `exceljs` (.xlsx) + `csv-parse` (.csv) — `xlsx`'s npm-published version has a live, unpatched high-severity prototype-pollution advisory with no fix available, which matters specifically here since this code parses untrusted, user-uploaded files. `exceljs`'s own bundled type declarations have an unrelated, cosmetic defect (see `backend/CLAUDE.md`'s Bulk Invite section) worked around with one commented `@ts-expect-error`, not a security concern.
