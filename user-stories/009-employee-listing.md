...
| Grade | This employee's `EmployeeCompanyAccess.gradeId` name ([004](./004-grade-management.md)) — added after this story shipped, at explicit request; see [Open Questions](#open-questions--assumptions) | Yes |
...
- **Grade column added after this story shipped, at explicit request**: [008](./008-employee-invitation.md)'s Step 2 (Company Access) already required and saved `roleId`/`departmentId`/`gradeId` together for both Invite and Edit from day one — this story's original table/API design table above only ever listed Role/Department as columns, so Grade was being collected and stored but never read back onto the listing. Fixed by mirroring the existing Role/Department join through `EmployeeCompanyAccess` (`backend/src/controllers/employee.controller.ts`'s `listEmployees`) and adding a `grade` field to the response, `EmployeeListItem`, the sortable-columns list, and the CSV export — same shape as Role/Department in every respect, including no dedicated filter input (it never had one either).
