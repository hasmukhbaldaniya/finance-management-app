...
**Out of scope for all of Story 1**: what happens on the invited employee's side (clicking the invite link, setting a password, completing their own account) — that's covered separately in [011-employee-onboarding.md](./011-employee-onboarding.md) (Story 4, added later at explicit request), structurally similar to [002](./002-organization-signup.md)'s registration wizard but triggered by an invite token instead of self-service signup. This story (and [009](./009-employee-listing.md)/[010](./010-bulk-invite-employees.md)) only cover the admin side.
...
| POST | `/api/employees/:id/approvals` | `{ approvers: [{ level, approverEmployeeId }] }` (no `moduleAccess` — removed after implementation, see [Open Questions](#open-questions--assumptions)) | `{ message }` |
| POST | `/api/employees/:id/send-invite` | — | `{ message }` (sends the invite email; also used the first time, not just for resends) |
| GET | `/api/employees` | — | `{ employees: [{ id, firstName, lastName, email }] }` (added during implementation, not in the original table — see [Open Questions](#open-questions--assumptions); the approver picker, scoped to the logged-in caller plus active employees whose Role has the "Claim / Trip Approvals" privilege) |
...
- The invited employee's own acceptance flow (invite link, setting a password, completing their profile) — now covered by [011-employee-onboarding.md](./011-employee-onboarding.md).
...
- **Invite/accept mechanism, resolved by [011](./011-employee-onboarding.md)**: `EmployeeInvite` remains a minimal send-log for the resend rate-limit/expiry rules [009](./009-employee-listing.md) needs — it was never extended into a full invite-token table. The actual accept-side token is a stateless JWT (mirroring `registrationToken`'s shape), issued fresh on every send/resend and verified independently by every onboarding endpoint; see [011](./011-employee-onboarding.md) for the full design. `sendEmployeeInviteEmail` (`utils/employee-invite-mailer.ts`) is no longer a `console.log` dev stub as of [011](./011-employee-onboarding.md) — it sends a real email through the same SMTP transporter `mailer.ts` already uses, now that there's a real onboarding link to put in it.
...
- **Module Access was removed entirely, at explicit request, after being built and then a second time after being re-scoped**: the original spec's Module Access checkbox group (Trips/Claims/Approvals/Finance/Reports) was implemented first — assumed module list `["trips", "claims", "approvals", "finance", "reports"]` matching [003](./003-header-navigation.md)'s nav items, fanned out into one `ApprovalLevel` row per `(selected module × level)` since that table is keyed `(employeeId, module, level)` — then removed from both frontend and backend. Step 4 ("Access & Approval") is now just "Approval" — a single approver chain, no module selection at all. `saveEmployeeApprovals` no longer accepts a `moduleAccess` field; every `ApprovalLevel` row it writes uses the fixed sentinel `module: "general"` (one row per approver level, not per module × level). The `ApprovalLevel.module` column itself is untouched in the schema (removing it would need a migration, out of scope for this change) — it's just no longer meaningful, user-facing data. If Module Access needs to come back later, re-adding the UI is straightforward, but the per-module fan-out logic would need to be re-derived, not just uncommented — it was deleted, not disabled.
