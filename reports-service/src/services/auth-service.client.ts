...
import { fetchAllPages, UpstreamError } from "./upstream-client";
...
export async function fetchAllEmployees(cookie: string): Promise<Map<number, EmployeeSummary>> {
  const [others, me] = await Promise.all([
    fetchAllPages<EmployeeSummary>(env.authServiceUrl, "/api/employees", "employees", cookie),
    fetch(new URL("/api/employees/me", env.authServiceUrl), { headers: { Cookie: cookie } }).then(async (response) => {
  ...
  const byId = new Map(others.map((employee) => [employee.id, employee]));
