...
import { Category, CategoryField, Claim, Expense, ExpenseSplitRequest, ExpenseSplitRequestMember, Trip } from "../models";
...
async function buildSplitRequestDetail(splitRequest: ExpenseSplitRequest, viewerId: number) {
  ...
  const employeeIds = Array.from(new Set([splitRequest.requestedByEmployeeId, ...members.map((member) => member.employeeId)]));
  ...
  const requester = employeeById.get(splitRequest.requestedByEmployeeId);
